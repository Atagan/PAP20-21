...
__global__ void convolucion(int* matrixImg, int* matrixFilter, int* matrixOut){
	...
	if (fila != 0 && columna != 0 && fila != WIDTH_IMG - 1 && columna != WIDTH_IMG - 1) { 
		...
		for (int j = -1; j < 2; j++) {
			for (int k = -1; k < 2; k++) {
				subMatriz[(j + 1) * WIDTH_FILTER + (k + 1)] = matrixImg[(fila * WIDTH_IMG + columna) + (j * WIDTH_IMG + k)];
			}
		}
	}
	else { 
		...
		for (int j = -1; j < 2; j++) {
			for (int k = -1; k < 2; k++) {
				...
				if (!((fila * WIDTH_IMG + columna + j * WIDTH_IMG + k) < 0 || (fila * WIDTH_IMG + columna + j * WIDTH_IMG + k) > WIDTH_IMG * WIDTH_IMG)) { // Si no se sale por arriba o por abajo dejamos el valor dde matrixImg
					subMatriz[(j + 1) * WIDTH_FILTER + (k + 1)] = matrixImg[(fila * WIDTH_IMG + columna) + (j * WIDTH_IMG + k)];                                 // Si se sale dejamos el 0 de calloc
				}
				else {
		...
		if ((fila * WIDTH_IMG + columna) % WIDTH_IMG == WIDTH_IMG - 1) { //Si la casilla en la que estamos se encuentra en la ultima columna de matrixImg, los valores de la ultima columna de subMatriz son 0
			//Aquí me encargo de las casillas por la derecha de la matrizImg
			for (int j = 1; j < 4; j++) {
				subMatriz[3 * j - 1] = 0;
			}
		}if ((fila * WIDTH_IMG + columna) % WIDTH_IMG == 0) { //Si la casilla en la que estamos se encuentra en la primera columna de matrixImg, los valores de la primera columna de subMatriz son 0
...
int main(){
	...
	convolucion <<< dimGrid, dimBlockImg >>> (matrizImg_d, matrizFTras_d, matrizOut_d);
